...
<html>
  <head>
	...
	#batch{font-size:15px;cursor: pointer;}
...
<table width="99%" border="0" cellspacing="0" cellpadding="0" id="searchmain">
  <tr>
    <td width="99%" align="left" valign="top">您的位置：管理员回收站</td>
  </tr>
  <tr>
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="0" id="search">
  		<tr>
   		 <td width="90%" align="left" valign="middle">
	         <form method="post" action="ManagerServlet?type=searchrubbish">
	         <span>管理员：</span>
	         <input type="text" name="manager_name" value="" class="text-word" placehloder="输入管理员名称">
	         <input name="" type="submit" value="查询" class="text-but">
	         </form>
         </td>
  		  <!-- <td width="10%" align="center" valign="middle" style="text-align:right; width:150px;"><a href="ManagerServlet?type=add" target="mainFrame" onFocus="this.blur()" class="add">新增管理员</a></td> -->
      ...
      <c:forEach items="${requestScope.managerlist}" var="manlist">
      	 <tr onMouseOut="this.style.backgroundColor='#ffffff'" onMouseOver="this.style.backgroundColor='#edf5ff'">
	        ...
	        <td align="center" valign="middle" class="borderbottom">
	        	...
	        	<a style="cursor: pointer;" target="mainFrame" onclick="clis(${manlist.manager_id })" onFocus="this.blur()" class="add">恢复</a><span class="gray">&nbsp;|&nbsp;</span>
      ...
      <c:if test="${requestScope.pg.count != 0}">	
	  		<tr id="ok">
		      	...
		      	<td class="align" style="height:24px;">
		      		<input name="type" type="submit" id="recover" value="恢复">
		      		<input name="type" type="submit" id="sub" value="删除">
  ...
  <tr>	
		...
		<c:if test="${requestScope.bool == '0' && requestScope.pg.count != 0}">	
  		<td align="left" valign="top" class="fenye"> 
		    ...
		    <a href="ManagerServlet?type=searchrubbish&currentpage=1&manager_name=${requestScope.name}" target="mainFrame" onFocus="this.blur()">首页</a>&nbsp;&nbsp;
		    <a href="ManagerServlet?type=searchrubbish&currentpage=<c:if test="${requestScope.pg.currentpage > 1}">${requestScope.pg.currentpage-1}</c:if><c:if test="${requestScope.pg.currentpage == 1}">${1}</c:if>&manager_name=${requestScope.name}" target="mainFrame" onFocus="this.blur()" name="lastpage">上一页</a>&nbsp;&nbsp;
		    <a href="ManagerServlet?type=searchrubbish&currentpage=<c:if test="${requestScope.pg.currentpage < requestScope.pg.pagecount}">${requestScope.pg.currentpage+1}</c:if><c:if test="${requestScope.pg.currentpage == requestScope.pg.pagecount}">${requestScope.pg.pagecount}</c:if>&manager_name=${requestScope.name}" target="mainFrame" onFocus="this.blur()" name="nextpage">下一页</a>&nbsp;&nbsp;
		    <a href="ManagerServlet?type=searchrubbish&currentpage=${requestScope.pg.pagecount }&manager_name=${requestScope.name}" target="mainFrame" onFocus="this.blur()">尾页</a>
		</td>
  	</c:if>
  	<c:if test="${requestScope.bool != '0' && requestScope.pg.count != 0}">
  		<td align="left" valign="top" class="fenye"> 
	    	${requestScope.pg.count }条数据 ${requestScope.pg.currentpage }/${requestScope.pg.pagecount } 页&nbsp;&nbsp;
		    <a href="ManagerServlet?type=rubbishlist&currentpage=1" target="mainFrame" onFocus="this.blur()">首页</a>&nbsp;&nbsp;
		    <a href="ManagerServlet?type=rubbishlist&currentpage=<c:if test="${requestScope.pg.currentpage > 1}">${requestScope.pg.currentpage-1}</c:if><c:if test="${requestScope.pg.currentpage == 1}">${1}</c:if>" target="mainFrame" onFocus="this.blur()" name="lastpage">上一页</a>&nbsp;&nbsp;
		    <a href="ManagerServlet?type=rubbishlist&currentpage=<c:if test="${requestScope.pg.currentpage < requestScope.pg.pagecount}">${requestScope.pg.currentpage+1}</c:if><c:if test="${requestScope.pg.currentpage == requestScope.pg.pagecount}">${requestScope.pg.pagecount}</c:if>" target="mainFrame" onFocus="this.blur()" name="nextpage">下一页</a>&nbsp;&nbsp;
		    <a href="ManagerServlet?type=rubbishlist&currentpage=${requestScope.pg.pagecount }" target="mainFrame" onFocus="this.blur()">尾页</a>
		</td>
  	</c:if>	
  	<td>
  		<c:if test="${requestScope.bool == '0' && requestScope.pg.count == 0}">	
...
<script type="text/javascript">
	function cli(id){
		if(confirm("是否彻底删除管理员")){
			window.location.href="ManagerServlet?type=delete&id="+id;
		}
	}
	function clis(id){
		if(confirm("是否恢复管理员")){
			window.location.href="ManagerServlet?type=recover&id="+id;
		}
	}
	
	var recover = document.getElementById("recover");
	recover.onclick = function(){
		if(confirm("是否批量恢复管理员")){
			return true;
		}else{
			return false;
		}
